...
The tool uses flow and workload data to identify a number of statistics
- What services are hosted and listening on a workload
- What is the associated risk of these services based on risk rating
- What potential exposure do these services have to other devices
- What active use do these services have based on flow data
- How many peers workloads have, both inbound and outbound
- How many flows exist for these relationships, revealing how active the services are and to/from where
- What mitigations could be applied to restrict services down to just those peers that are active
...
- Identify app peer relationships, relationship counts and flow counts
...
1) asprequests.py - a wrapper for the Illumio API that simplifies query to the API, Async API and the Async Traffic API. This has a dependancy on the Python requests library
